...
var __keys$2 = require('../__keys-0c8e6398.js');

function useDisclosureRef(dialogRef, options) {
  var ref = React.useRef(null);
  React.useEffect(function () {
    if (options.visible) return undefined; // We get the last focused element before the dialog opens, so we can move
  ...
  React.useEffect(function () {
    ...
    if (!options.visible) return undefined; // Safari and Firefox on MacOS don't focus on buttons on mouse down.
...
function useEventListenerOutside(containerRef, disclosureRef, nestedDialogs, eventType, listener, shouldListen) {
...
function useHideOnClickOutside(dialogRef, disclosureRef, nestedDialogs, options) {
  ...
  useEventListenerOutside(dialogRef, disclosureRef, nestedDialogs, "focus", function (event) {
